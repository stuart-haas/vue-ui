<template>
  <tr class="border-b border-gray-300">
    <td colspan="9">
      <div class="bg-gray-900 rounded my-4">
        <div class="grid grid-cols-2 gap-4">
          <div class="p-4" v-if="commitList && commitList.length">
            <div class="flex items-center justify-between mb-4">
              <p class="font-semibold text-green-500">Commit History</p>
              ...
              <div v-for="(item, index) in commitList" :key="index">
                <div class="inline-block cursor-pointer">
                  <div
                    ...
                    class="bg-slate-700 rounded p-2 hover:bg-slate-600 transition-color duration-200 flex items-center space-x-2"
                    :class="{ 'bg-slate-600': activeIndex == index }"
                  >
                    <span class="text-white text-sm">{{
                      item.commit.message
                    }}</span>
                    <Popover>
                      ...
                      >
                        <span class="text-blue-300 text-xs text-right"
                          >Authored by {{ item.author.login }}
                          {{ getDate(item?.commit?.author.date) }}</span
                        >
                      </div>
                    </Popover>
                  </div>
          ...
          <div>
            <div v-if="commit" class="bg-gray-800 h-full">
              <header
                class="bg-slate-700 mb-2 p-4 flex items-center justify-between"
              >
                <div class="flex items-center space-x-4">
                  ...
                  >
                    ...
                    <i class="fas fa-link ml-2" />
                  </a>
                </div>
                <div class="flex items-center space-x-4">
                  <p class="text-sm text-gray-300">
                    Authored by {{ commit.author.login }}
                    {{ getDate(commit?.commit?.author.date) }}
                  </p>
                </div>
              </header>
...
async function fetchCommitList() {
  ...
  const response = await axios.get(
    `https://api.github.com/repos/${props.item.owner.login}/${props.item.name}/commits?page=${page.value}&per_page=10`,
    {
      headers: {
